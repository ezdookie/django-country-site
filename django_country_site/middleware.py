...
from .utils import (
    get_countries_and_language, get_country_from_path, get_language_country_from_request
)
...
class CountryLocaleMiddleware(object):
    ...
    def process_request(self, request):
        ...
        country_code = get_language_country_from_request(request)

        if country_code in supported_country_codes:
            activate_country(country_code)
            set_country(country_code, request)

            request.session[LANGUAGE_SESSION_KEY] = supported_country_codes[country_code]
    ...
    def process_response(self, request, response):
        ...
        if (response.status_code == 404 and not country_from_path
            ...
            country_path = '/%s%s' % (country, request.path_info)
